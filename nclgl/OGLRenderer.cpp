...
void OGLRenderer::SetShaderLight(const std::vector<Light>& l) {
	memset(lightColours, 0, sizeof(float) * 8);
	memset(lightPosition, 0, sizeof(float) * 6);

	for (size_t j = 0; j < 8; j++) {
		size_t offset = j % 4;
		size_t dim = j / 4;
		float colPart = l[dim].getColour()[offset];
		lightColours[j] = colPart;
	}
	for (size_t j = 0; j < 6; j++) {
		size_t offset = j % 3;
		size_t dim = j / 3;
		float colPart = l[dim].getPosition()[offset];
		lightPosition[j] = colPart;
	}

	for (size_t i = 0; i < 2; i++)
		lightRadius[i] = l[i].getRadius();
	for (size_t i = 0; i < 2; i++) {
		if (l[i].getDirectional()) {
			directional[i] = 1;
		}
		else {
			directional[i] = 0;
	...
	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "lightPos"), 2, lightPosition);
	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(), "lightColour"), 2, lightColours);
	glUniform1fv(glGetUniformLocation(currentShader->GetProgram(), "lightRadius"), 2, lightRadius);
	glUniform1fv(glGetUniformLocation(currentShader->GetProgram(), "directional"), 2, directional);
	
}
void  OGLRenderer::SetShaderLightSingle(const  Light& l) {
