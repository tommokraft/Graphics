...
Mesh* Mesh::GenerateQuad(float size) {
	...
	if (size == 0.25f) {
		m->vertices[0] = Vector3(0.5, 1.0f, 0.0f);
		m->vertices[1] = Vector3(0.5,0.5, 0.0f );
...
void	Mesh::BufferData()	{
	...
	if (weightIndices) {
		...
		glBufferData(GL_ARRAY_BUFFER, numVertices * sizeof(int) * 4, weightIndices, GL_STREAM_DRAW);
	...
	if(indices) {
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, numIndices*sizeof(GLuint), indices, GL_STREAM_DRAW);
